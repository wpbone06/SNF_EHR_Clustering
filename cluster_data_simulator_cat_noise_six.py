...
################################################################################
## cat3: Generate 6 level categorical values from continuous values

################################################################################
def cat3(vector):

    return pd.qcut(vector, [0, .166, .333, .50, .666, .833,  1]).codes
...
def run_sim(n_samples, sig_cont, sig_cat, cat_noise_feat, cont_noise_feat, clusters, seed):
    ...
    name = '_{}_{}_{}_{}_{}_{}_{}'.format(n_samples, sig_cont, sig_cat, cont_noise_feat, cat_noise_feat, seed, clusters)
...
def main():
    ...
    if len(argsList) != 12:
        print "\nYou have supplied too many or too few arguments!\nAccepted arguments are:\n\n-n n_samples -x sig_cont -y sig_cat -w cat_noise_feat -z cont_noise_feat -s seed"
        print helpstatement
        exit(1)
    ...
    ## Parsing arguments from command line
        ...
        try:
            opts, args = getopt.getopt(argsList,"n:x:y:w:z:s:")

        except:
            print("Usage: %s -n n_samples -x sig_cont -y sig_cat -w cat_noise_feat -z cont_noise_feat -s seed" % sys.argv[0])
            print helpstatement
            exit(1)
    ...
    #Currently script only generates 6 cluster simulated data
