...
do echo $file
    ...
    echo $file $silVal >> ./sample_size_results_silhouette_values_SNFtool_X_$fileName.txt
done

echo ./sample_size_results_silhouette_values_SNFtool_X_$fileName.txt

# remove the file names except for the seed number and the clusters number
cat ./sample_size_results_silhouette_values_SNFtool_X_$fileName.txt | tr " " "\t" | sed "s/.d/_/" | tr "_" "\t" | cut -f 5,11,17 | sort -nk 2 > ./sample_size_results_silhouette_values_SNFtool_X_"$fileName"_dataframe.txt
...
echo "./sample_size_results_silhouette_values_SNFtool_X_"$fileName"_dataframe.txt file complete"
