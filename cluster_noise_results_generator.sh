...
do echo $file
    ...
    echo $file $silVal >> ./noise_results_silhouette_values_SNFtool_X_$fileName.txt
done

echo ./noise_results_silhouette_values_SNFtool_X_$fileName.txt

# remove the file names except for the seed number and the clusters number
cat ./noise_results_silhouette_values_SNFtool_X_$fileName.txt | tr " " "\t" | sed "s/.d/_/" | tr "_" "\t" | cut -f 8,11,17 | sort -nk 2 > ./noise_results_silhouette_values_SNFtool_X_"$fileName"_dataframe.txt
...
echo "./noise_results_silhouette_values_SNFtool_X_"$fileName"_dataframe.txt file complete"
