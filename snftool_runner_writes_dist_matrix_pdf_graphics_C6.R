...
#snftool_runner.sh
#NOTE!!!!: script currently assumes you are providing a cluster label file with 6 clusters
...
print("NOTE!!!!: script currently assumes you are providing a cluster label file with 6 clusters!")
...
print("group.dum = spectralClustering(W.dum,6)")
print("SNFcluster.dum <- as.data.frame(group.dum)")
group.dum = spectralClustering(W.dum,6)
SNFcluster.dum <- as.data.frame(group.dum)

#Concatenate the true labels with the called labels by the Spectral Clustering on the SNFtool clustered data
