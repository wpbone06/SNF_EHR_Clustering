...
################################################################################
## cat3: Generate the cluster number specified in -c level categorical values from continuous values
...
################################################################################
def make_data(n_samples, sig_cont, sig_cat, cat_noise_feat, cont_noise_feat, clusters, seed, remove_n):
    ...
    #eenerate the noise features from a uniform distribution
    ...
    #remove samples from group 0 in X_data
    #################################################################################
    X_data_df = pd.DataFrame(X_data)

    y_vector_df = pd.DataFrame(y_vector)
    y_vector_df.columns = ["labels"]

    #add the cluster labels to the X_data dataframe
    X_data_labeled = pd.concat([y_vector_df, X_data_df], axis = 1)

    #split data into 0 cluster and other clusters dataframes
    X_data_0_df  = X_data_labeled.loc[X_data_labeled["labels"] == 0]

    X_data_not0_df  = X_data_labeled.loc[X_data_labeled["labels"] != 0]

    #list of sample indices to remove
    drop_samples = np.random.choice(X_data_0_df.index, remove_n, replace=False)

    #drop samples from X_data the y_vector
    X_data_0_df = X_data_0_df.drop(drop_samples)
    y_vector_df = y_vector_df.drop(drop_samples)

    X_data_df =  pd.concat([X_data_0_df , X_data_not0_df], axis =0)

    #remove the labels
    X_data_df = X_data_df.drop("labels", axis =1)

    #pdb.set_trace()
    #return the data to numpy array format
    X_data = X_data_df.as_matrix()

    y_vector_df = y_vector_df.transpose()
    y_vector = y_vector_df.as_matrix()
...
################################################################################
def run_sim(n_samples, sig_cont, sig_cat, cat_noise_feat, cont_noise_feat, clusters, seed, remove_n):

    X_data, y_vector, cat_bool, X_dummy_data = make_data(n_samples, sig_cont, sig_cat, cat_noise_feat, cont_noise_feat, clusters, seed, remove_n)

    name = '_{}_{}_{}_{}_{}_{}_{}'.format(n_samples, sig_cont, sig_cat, cont_noise_feat, cat_noise_feat, clusters, seed)
...
def main():
    ...
    helpstatement = "\n\nThis script will output the simulation files in the directory you run it from. It requires that you supply it with the number of samples, the number of signal continuous features, the number of signal categorical features, the number of noise features, the number of EVENLY SIZED clusters you wish to generate, the number of samples you would like to remove from cluster 0, and a seed in case you wish to regenerate the same data.\n"

    if len(argsList) != 16:
        print "\nYou have supplied too many or too few arguments!\nAccepted arguments are:\n\n-n n_samples -x sig_cont -y sig_cat -w cat_noise_feat -z cont_noise_feat -s seed -c # of clusters -r # of samples to remove from cluster 0"
        print helpstatement
        exit(1)
    ...
    ## Parsing arguments from command line
        ...
        try:
            opts, args = getopt.getopt(argsList,"n:x:y:w:z:s:c:r:")

        except:
            print("Usage: %s -n n_samples -x sig_cont -y sig_cat -w cat_noise_feat -z cont_noise_feat -s seed -c # of clusters -r # of samples to remove from cluster 0" % sys.argv[0])
            print helpstatement
            exit(1)
    ...
    for (opt, arg) in opts:
        ...
        if opt == '-n':
            ...
            print "Number of evenly sized clusters %s\n" %cluster_str

        elif opt == '-r':
            remove_n_str = arg
            remove_n = int(remove_n_str)

            print "Number of samples to remove from cluster 0 %s\n" %remove_n_str
    ...
    run_sim(n_samples, sig_cont, sig_cat, cat_noise_feat, cont_noise_feat, clusters, seed, remove_n)
